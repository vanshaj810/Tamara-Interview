...
describe('Repayment Plan API', () => {
    describe('POST /api/v1/repayment-plan', () => {
        it('should create a repayment plan for valid request', async () => {
            ...
            expect(response.body.profit_margin_percentage).toBe(12); // Default 12%
        ...
        it('should apply SAVE10 promo code', async () => {
            ...
            expect(response.body.total_payable).toBe(1108);
        });

        it('should apply SAVE20 promo code', async () => {
            const response = await request(app)
                .post('/api/v1/repayment-plan')
                .send({
                    customer_id: 'CUST004',
            ...
            expect(response.status).toBe(201);
            // 12% * 0.8 = 9.6%
            expect(response.body.profit_margin_percentage).toBe(9.6);
        });
        ...
        it('should handle month-end date in schedule', async () => {
            ...
            expect(response.status).toBe(201);
            // First installment should be Feb 28 (not 31)
        ...
        it('should include proper contract summary', async () => {
            ...
            expect(response.body.contract_summary).toContain('SAR 1120.00');
            expect(response.body.contract_summary).toContain('SAR 120.00');
            expect(response.body.contract_summary).toContain('4 equal monthly installments');
        });
    });
